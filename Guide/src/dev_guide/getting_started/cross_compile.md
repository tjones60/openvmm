...
if [[ $1 == "build" || $1 == "run" ]]; then
    ...
    if [[ $3 == "vmm" ]]; then
        ...
        if [[ $4 == "uefi" ]]; then
            args+=" --uefi firmware=$uefi_firmware --vmbus-scsi id=scsi0 --disk memdiff:$disk_path,on=scsi0"
        elif [[ $4 == "linux" ]]; then
            args+=""
        else
        ...
        if [[ $4 == "uefi" ]]; then
            recipe="$short_arch"
            args+=" --vmbus-scsi id=scsi0 --disk memdiff:$disk_path,on=scsi0 --gfx --vmbus-com1-serial term --uefi console=com1"
        elif [[ $4 == "linux" ]]; then
...
active, you can omit the `firmware` option. The `X86_64_OPENVMM_UEFI_FIRMWARE`
